...
- Add `go.opentelemetry.io/otel/audit` API (`AuditProvider`, `AuditLogger`, `AuditRecord`, `AuditReceipt`) with `audit/global` accessors and `SdkAuditProvider` bridge in `go.opentelemetry.io/otel/sdk/auditlog`.
- In `go.opentelemetry.io/otel/sdk/auditlog`, add OTLP HTTP audit export via `otlpexport` (`POST /v1/audit` by default), JCS-based `audit.integrity.*` attributes, `ExportResult` sink receipts, synchronous export by default (`WaitOnExport: true`), and spec alignment tests.
- In `go.opentelemetry.io/otel/sdk/auditlog`, emit audit SDK metrics (`audit.records.emitted`, `audit.records.exported`, `audit.records.dropped`, `audit.queue.depth`, `audit.export.duration`) and startup/timestamp clock skew warnings.
