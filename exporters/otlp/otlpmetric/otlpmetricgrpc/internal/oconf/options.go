...
func WithFallbackEndpoint(endpoint string) GenericOption {
	return newGenericOption(func(cfg Config) Config {
		cfg.Metrics.FallbackEndpoint = endpoint
		return cfg
	})
}
...
func WithFallbackEndpointURL(v string) GenericOption {
	return newGenericOption(func(cfg Config) Config {
		u, err := url.Parse(v)
		if err != nil {
			global.Error(err, "otlpmetric: parse fallback endpoint url", "url", v)
			return cfg
		}

		cfg.Metrics.FallbackEndpoint = u.Host
		cfg.Metrics.FallbackURLPath = u.Path
		cfg.Metrics.FallbackURLPathFromURL = true
		if cfg.Metrics.FallbackURLPath == "" {
			cfg.Metrics.FallbackURLPath = "/"
		}
		cfg.Metrics.FallbackInsecure = u.Scheme != "https"
